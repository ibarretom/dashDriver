...
import { useCarRides } from "../hooks/carRides";

import { months } from "../utils/months";

export function CarRidesResume() {
  const { getMostVisitedPlaces } = useCarRides()
  ...
  useEffect(() => {
    getMostVisitedPlaces({ month, date })
      .then(d => setMostVisitedPlaces(d))
  ...
  return (
    <ScrollView>
      ...
      {mostVisitedPlaces.mostVisited.length > 0 ?
        mostVisitedPlaces.mostVisited.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
      ...
      {mostVisitedPlaces.mostVisitedByMonth.length > 0 ?
        mostVisitedPlaces.mostVisitedByMonth.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
        <Text style={styles.emptyRide}>Nenhuma corrida registrada para este mês</Text>
      }

      <Text style={styles.title}>Locais com mais corridas por dia</Text>

      {mostVisitedPlaces.mostVisitedByDay.length > 0 ?
        mostVisitedPlaces.mostVisitedByDay.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
        <Text style={styles.emptyRide}>Nenhuma corrida registrada para este dia</Text>
      }

      <Text style={styles.title}>Locais com mais corridas por horario</Text>

      <Text style={styles.subTitle}>6:00 até 12:00</Text>

      {mostVisitedPlaces.mostVisitedMorning.length > 0 ?
        mostVisitedPlaces.mostVisitedMorning.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
        <Text style={styles.emptyRide}>Nenhuma corrida registrada para este dia</Text>
      }
      ...
      {mostVisitedPlaces.mostVisitedAfternoon.length > 0 ?
        mostVisitedPlaces.mostVisitedAfternoon.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
        <Text style={styles.emptyRide}>Nenhuma corrida registrada para este dia</Text>
      }
      ...
      {mostVisitedPlaces.mostVisitedNight.length > 0 ?
        mostVisitedPlaces.mostVisitedNight.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
        <Text style={styles.emptyRide}>Nenhuma corrida registrada para este dia</Text>
      }
      ...
      {mostVisitedPlaces.mostVisitedDawn.length > 0 ?
        mostVisitedPlaces.mostVisitedDawn.map((place, i) => <PlaceCard key={i} title={place.id} value={place.data} />) :
        <Text style={styles.emptyRide}>Nenhuma corrida registrada para este dia</Text>
      }
