...
export function Spent({ navigation }) {
  ...
  async function handleSpentSubmit({ spentType, amount, date, description }) {
    setIsLoading(true)

    try {
      await addDoc(collection(db, 'spent'), {
        user_id: user.uid,
      ...
      Alert.alert('Despesa adicionada', 'Despesa adicionanda com sucesso')

      setIsLoading(false)

    } catch (e) {
      Alert.alert('Erro ao adicionar despesa', 'Não foi possível adicionar despesa')
